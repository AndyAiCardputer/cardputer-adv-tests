 ...
 */
 ...
 static const st7123_lcd_init_cmd_t vendor_specific_init_default[] = {
     ...
     {0x36, (uint8_t []){0x20}, 1, 0},  // Rotate 90° left WITHOUT mirroring (landscape orientation)
...
static esp_err_t panel_st7123_init(esp_lcd_panel_t *panel)
{
    ESP_LOGI(TAG, "panel_st7123_init: Starting initialization...");
    st7123_panel_t *st7123 = (st7123_panel_t *)panel->user_data;
    ...
    uint16_t init_cmds_size = 0;
     ...
     // Skip reading ID - causes hang on some displays
     // uint8_t ID[3];
     ...
     ESP_LOGI(TAG, "Skipping ID read (known to cause hang)");

     // Wait a bit for MIPI DSI bus to stabilize
     ESP_LOGI(TAG, "Waiting for MIPI DSI bus to stabilize...");
     ...
     ESP_LOGI(TAG, "Starting to send %d initialization commands...", init_cmds_size);
     for (int i = 0; i < init_cmds_size; i++) {
         ...
         // Log each command being sent
         ESP_LOGI(TAG, "Sending init command %d/%d: cmd=0x%02X, data_size=%d, delay_ms=%d", 
                  i+1, init_cmds_size, init_cmds[i].cmd, init_cmds[i].data_bytes, init_cmds[i].delay_ms);

         // Send command
         esp_err_t cmd_ret = esp_lcd_panel_io_tx_param(io, init_cmds[i].cmd, init_cmds[i].data, init_cmds[i].data_bytes);
         if (cmd_ret != ESP_OK) {
             ESP_LOGE(TAG, "Failed to send command 0x%02X: %s", init_cmds[i].cmd, esp_err_to_name(cmd_ret));
             return cmd_ret;
         }
         ESP_LOGI(TAG, "Command 0x%02X sent successfully", init_cmds[i].cmd);
         
         if (init_cmds[i].delay_ms > 0) {
             ESP_LOGI(TAG, "Waiting %d ms after command 0x%02X", init_cmds[i].delay_ms, init_cmds[i].cmd);
             vTaskDelay(pdMS_TO_TICKS(init_cmds[i].delay_ms));
         } else {
             // Add small delay even if delay_ms is 0 to prevent bus overload
     ...
     ESP_LOGI(TAG, "All init commands sent successfully");

     ESP_LOGI(TAG, "Initializing MIPI DPI panel...");
     ESP_RETURN_ON_ERROR(st7123->init(panel), TAG, "init MIPI DPI panel failed");
     ESP_LOGI(TAG, "MIPI DPI panel initialized successfully");
