...
void bsp_set_charge_en(bool en)
{
    ...
    // Verify charge enable bit was set correctly
    write_buf[0] = PI4IO_REG_OUT_SET;
    i2c_master_transmit_receive(i2c_dev_handle_pi4ioe2, write_buf, 1, read_buf, 1, I2C_MASTER_TIMEOUT_MS);
    bool actual_state = (read_buf[0] & 0b10000000) != 0;
    ESP_LOGI("M5STACK_TAB5", "Charge enable: requested=%d, actual=%d, register=0x%02X", 
             en, actual_state, read_buf[0]);
}
...
bool bsp_get_charge_en(void)
{
    ...
    if (ret != ESP_OK) {
        ESP_LOGW("M5STACK_TAB5", "Failed to read charge enable status: %s", esp_err_to_name(ret));
        return false;
    }
    
    // Bit 7 = CHG_EN
    return (read_buf[0] & 0b10000000) != 0;
}
