...
// Note: Specification shows 0x40, but actual device responds on 0x41
// Device at 0x40 (CONFIG: 36 FF) appears to be ES7210 microphone
// Device at 0x41 (CONFIG: 41 27) is INA226
...
typedef struct {
    ...
    bool battery_present; // True if battery is detected
...
/**
 * @brief Get battery current in milliamperes (signed)
 * 
 * @param out_ma Pointer to store current in mA (negative = charging, positive = discharging)
 * @return ESP_OK on success
 */
esp_err_t battery_monitor_get_current_ma(float *out_ma);

/**
 * @brief Get shunt voltage in microvolts (signed)
 * 
 * @param out_uv Pointer to store shunt voltage in µV (can be negative)
 * @return ESP_OK on success
 */
